...
This command reads `instructions/delegate.md` from the package directory and writes it to stdout. Because `delegate` is an **instruction skill**, this is its primary surface — there are no domain commands. The agent applies the methodology with its own `aux4 jobs` / `cron` / `poll` / `queue` calls; the prompt does not run jobs or call an LLM itself.
